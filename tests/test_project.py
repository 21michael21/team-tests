...
class TestProject:
    ...
    def test_create_project_success(self, teamcity_client, data_generator, mock_requests):
        # Arrange
        project_data = data_generator.generate_project_data()
        mock_url = f"{teamcity_client.base_url}/app/rest/projects"
        project_response = mock_project_response(project_data)  # Изменили mock_project_response на project_response
        mock_requests.post(mock_url, json=project_response["json"], status_code=project_response["status_code"])
    ...
    def test_create_project_duplicate_id(self, teamcity_client, data_generator, mock_requests):
        # Arrange
        project_data = data_generator.generate_project_data()
        mock_url = f"{teamcity_client.base_url}/app/rest/projects"
        error_response = mock_project_error("Project ID already exists", status_code=409)  # Изменили mock_project_error на error_response
        mock_requests.post(mock_url, json=error_response["json"], status_code=error_response["status_code"])
    ...
    def test_create_project_invalid_data(self, teamcity_client, invalid_data, mock_requests):
        # Arrange
        mock_url = f"{teamcity_client.base_url}/app/rest/projects"
        error_response = mock_project_error("Invalid project data", status_code=400)  # Изменили mock_project_error на error_response
        mock_requests.post(mock_url, json=error_response["json"], status_code=error_response["status_code"])
